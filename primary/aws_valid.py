...
API_ENDPOINT_DEEPGRAM_CALLBACK = "https://lsehufc3n2.execute-api.us-west-2.amazonaws.com/api/transcription"
...
API_ENDPOINT_HMAC_HASH_PROD = "https://2hk294prkd.execute-api.us-west-2.amazonaws.com/prod/generate-hash"
...
API_ENDPOINT_HASH_STORE_PROD = "https://d3aaq3eeyj.execute-api.us-west-2.amazonaws.com/prod/hash-store"
SCHEMA_HASH_STORE = {
    "$schema": "http://json-schema.org/draft-04/schema#",
...
API_ENDPOINT_QRAG_ROUTING_PROD = "https://va6mkwd7p3.execute-api.us-west-2.amazonaws.com/prod/qrag-routing"
SCHEMA_QRAG_ROUTING = {
    "$schema": "http://json-schema.org/draft-04/schema#",
...
API_ENDPOINT_QRAG_LLM_PROD = "https://48jjmxdbg2.execute-api.us-west-2.amazonaws.com/prod/qrag-llm"
SCHEMA_QRAG_LLM = {
    "$schema": "http://json-schema.org/draft-04/schema#",
...
API_ENDPOINT_SEND_EMAIL_PROD = "https://vbb9ybb4p1.execute-api.us-west-2.amazonaws.com/prod/send-email"
SCHEMA_SEND_EMAIL = {
    "$schema": "http://json-schema.org/draft-04/schema#",
...
API_ENDPOINT_VRAG_LLM_PROD = "https://wxehiew7od.execute-api.us-west-2.amazonaws.com/prod/vrag-llm"
SCHEMA_VRAG_LLM = {
    "$schema": "http://json-schema.org/draft-04/schema#",
...
# ===== MAIN SECTION OF FILE primary/aws-valid.py =====
...
CHALICE_FOLDER = "web/aws_chalice/"
...
def test_lambda_requests(app_name, env, first_request_only=False, direct_lambda=True, with_gateway=True, debug_prompt=False, output_file="web/test_back-end_validation.md", jwt_token=None):
    """
...
### PROD STAGE
PROMOTE_TO_PROD_ZIP_PATH = "web/aws_chalice/qrag-llm/.chalice/deployments/prod-init-promote_2025-02-27_070200_b043a92fd1d38a85e9db0e99138d3633-python3.11.zip"
...
def mrun_rename_api_gateway():
    pass
#if __name__ == "__main__":
    rename_api_gateway("48jjmxdbg2", "qrag-llm-prod")
...
def mrun_rename_api_gateway_stage():
    pass
#if __name__ == "__main__":
    rename_api_gateway_stage("2hk294prkd", "api", "prod")
...
Copying role from dev to prod for: send-email
  ...
  ✅ Role ARN: arn:aws:iam::957789311461:role/send-email-prod
...
✅ Successfully created prod role send-email-prod copied from send-email-dev
  Prod role ARN: arn:aws:iam::957789311461:role/send-email-prod
...
def create_shared_role_for_lambda(app_name, env='dev', configure_api_gateway=True):  # DEPRECATED
    ...
    try:
        ...
        if configure_api_gateway:
            try:
                from primary.aws import get_api_gateway_and_resource_ids
...
def mtest_get_app_dot_py_code_from_zip():
    pass
#if __name__ == "__main__":
    zip_path = "web/aws_chalice/vrag-llm/.chalice/deployments/prod-init-promote_f115601e24db7e859c605b4178f8cd1c-python3.11.zip"
...
def get_last_deployed_log_file_paths(app_name):
    ...
    """
    dev_logs_dir = f"{CHALICE_FOLDER}{app_name}/deployed_dev_logs"
    prod_logs_dir = f"{CHALICE_FOLDER}{app_name}/deployed_prod_logs"
    ...
    for i, (api_name, api_section) in enumerate(api_sections):
        ...
        if not api_name.endswith('-prod'):
            ...
            if prod_idx != -1:
                ...
                # Format the logs section
                logs_section = "\n## last deployed logs\n"
                
                # Format dev log
                if dev_log_path:
                    dev_filename = os.path.basename(dev_log_path)
                    dev_relative_path = f"../../web/aws_chalice/{api_name}/deployed_dev_logs/{dev_filename}"
                ...
                if prod_log_path:
                    ...
                    prod_relative_path = f"../../web/aws_chalice/{api_name}/deployed_prod_logs/{prod_filename}"
        ...
        for api_gateway_name in api_gateway_names:
            ...
            for stage_name in stages_to_process:
                ...
                if log_path:
                    ...
                    relative_path = f"../../web/aws_chalice/{app_name}/deployed_{env_type}_logs/{filename}"
...
def deploy_lambda_zip(zip_path, target_lambda_function_name, env_variable_changes=None, publish_version=False, version_description=None, logger=None):
    ...
    if logger:
        ...
        logger.log(f"  env_variable_changes: {env_variable_changes}")
...
def mtest_deploy_lambda_zip():
    pass
#if __name__ == "__main__":
    zip_path = 'web/aws_chalice/hmac-hash/.chalice/deployments/2025-04-04_063224_9cc1732dfcca94718655e284f7ee1d2c-python3.11.zip'
    prod_log_path = 'web/aws_chalice/hmac-hash/deployed_prod_logs'
...
def mtest_get_lambda_zip_from_deployed_log():
    pass
#if __name__ == "__main__":
    log_file_path = 'web/aws_chalice/hmac-hash/deployed_dev_logs/deployed_dev_log_2025-04-06_114140.md'
    zip_path = get_lambda_zip_from_deployed_log(log_file_path)
    print(f"zip_path: {zip_path}")

PROMOTE_PROD_APP_NAME = cur_app_name
COMPOSITE_LOG_FILE_PATH = "web/aws_chalice/chalicelib_mirror_deploy_composite_log.md"
def fix_post_promote_api_state_report(app_name, post_promote_api_state_report_file_path):
    ...
    try:
        ...
        relative_path = f"../../web/aws_chalice/{app_name}/deployed_prod_logs/{latest_prod_log_filename}"
...
def promote_to_prod(app_name, version_description, zip_path=None):
    ...
    # Setup log paths early
    prod_log_folder = chalice_app_folder + "/deployed_prod_logs"
    datetime = get_current_datetime_filefriendly()
...
if __name__ == "__main__":
    ...
    if zip_path is not None:
        ...
        import re
        match = re.search(r'web/aws_chalice/([^/]+)/', zip_path)
        if match:
