...
def remove_from_file_list(folder_path, base_file_list):
    ...
    """
    from chalicelib.fileops import get_files_in_folder
...
def get_api_gateway_ids(api_name, http_method, stage='dev'):
    """
    ...
    try:
        ...
        rest_api_id = api['id']
        print(f"Found API '{api_name}' with ID: {rest_api_id}")
        ...
        print(f"No resource found with {http_method} method for API: {api_name}")
        return None, None
        
    except ClientError as e:
        print(f"Error getting API Gateway IDs: {e}")
        return None, None
def mtest_get_api_gateway_ids():
    pass
#if __name__ == "__main__":
    api_gateway_name = "deepgram-callback"
    print(get_api_gateway_ids(api_gateway_name, "GET"))
...
if __name__ == "__main__":
    ...
    token = generate_jwt("test-subject", 1)
