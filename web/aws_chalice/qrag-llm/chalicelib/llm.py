...
### ADD SECTIONS - OLD
def get_transcript_speaker_lines(file_path, remove_speakers, remove_duplicates=False, verbose=False):
    """
...
def mtest_get_transcript_speaker_lines():
    ...
    speaker_lines = get_transcript_speaker_lines(cur_file_path, remove_speakers=["FDA"], remove_duplicates=True, verbose=True)
...
def mrun_add_transcript_section_delimiters():
    ...
    speaker_tuples = get_transcript_speaker_lines(cur_file_path, remove_speakers=["FDA"], remove_duplicates=True)
...
def mrun_add_transcript_section_delimiters_folder():
    ...
    for i, file_path in enumerate(files_to_run, 1):
        speaker_tuples = get_transcript_speaker_lines(file_path, remove_speakers=["FDA"], remove_duplicates=True)
        # Extract line numbers and subtract 1 from each
        prev_line_numbers = [line_num - 1 for _, line_num in speaker_tuples]
        print(f"prev_line_numbers: {prev_line_numbers}")
