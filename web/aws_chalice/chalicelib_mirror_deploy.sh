...
echo "Running chalicelib_mirror_deploy.sh - LAST UPDATED: 4-13 0800 change composite logging to call add_line_to_top_of_file_but_below_header_lines"
# PREVIOUS LAST UPDATED: 4-2 0450 cleaning up prints
echo "Deploying to Chalice folder: $CHALICE_FOLDER, environment: $ENV"
...
cd ../../../
python3 -c "
from primary.aws_valid import set_request_validation, APIS_VALIDATION_ENABLED, UnifiedLogger, map_names_env_stages
import boto3
import time
import os
...
if [ "$ENV" == "dev" ]; then
    # Create the dev logs directory if it doesn't exist
    DEV_LOG_DIR="./deployed_dev_logs"
    mkdir -p "$DEV_LOG_DIR"
...
from primary.aws_valid import extract_last_updated_comment
import sys
import os
...
from primary.aws_valid import generate_api_state_report
import os
...
")
    ...
    # Add the link to the API state report file with correct relative path
    if [ -n "$API_REPORT_OUTPUT" ]; then
        echo "[$REPORT_FILENAME](../../../../$API_STATE_REPORT_FOLDER/$REPORT_FILENAME)" >> "$DEV_LOG_FILE"
    else
...
# Log the deployment to the composite deployment log file
COMPOSITE_LOG_FILE="../chalicelib_mirror_deploy_composite_log.md"
APP_PY="./app.py"
...
if [ ! -f "$COMPOSITE_LOG_FILE" ]; then
    ...
    echo "<START OF FILE web/aws_chalice/chalicelib_mirror_deploy_composite_log.md>" > "$COMPOSITE_LOG_FILE"
...
if [ "$ENV" == "prod" ]; then
    ...
    # For dev, create markdown link to the individual log file
    DEV_LOG_FILENAME="deployed_dev_log_$TIMESTAMP.md"
    DEV_LOG_PATH="web/aws_chalice/${app_name}/deployed_dev_logs/$DEV_LOG_FILENAME"
...
python3 -c "
import sys
from primary.aws_valid import add_line_to_top_of_file_but_below_header_lines
...
if [ -d "$DEPLOYMENT_DIR" ]; then
    ...
    if [ -n "$LATEST_DEPLOYMENT" ]; then
        ...
        # Construct full relative path for display and logging
        # Starting from web/aws_chalice/[app_name]
        FULL_RELATIVE_PATH="web/aws_chalice/$CURRENT_DIR/$DEPLOYMENT_DIR/$NEW_FILENAME"
