...
### YOUTUBE FUNCTIONS
def download_mp3_from_youtube(url, output_title='downloaded_audio'):  # DS, unittests 1
    """ 
    ...
    :category: 3b non ffop file function - does not have an input file but produces or modifies an output file
    :area: YOUTUBE FUNCTIONS
    :usage: print(download_mp3_from_youtube(cur_url, output_title = cur_path + '/download_yt_test'))
    """
...
def get_youtube_title_length(url):  # unittests 1
    """
    Retrieves the title and length (duration) of a YouTube video in our tuned timestamp format. Uses yt_dlp package.
    Tuned timestamp format omits H if none and leading M zero.

    :return: Tuple containing the video title and its duration as a string in our tuned timestamp format.
    """
    from general.fileops import tune_timestamp
    ydl_opts = {
        'quiet': True,
...
def download_link_list_to_mp3s(links, audio_inbox_path="data/audio_inbox"):  # unittests 1 NO CALLERS (3-3 RT)
    """
    ...
    # DONE: Wrap this up into a function where you pass in only the list of links
    # DONE: Remove any file extensions at the end of title so that the final mp3 only has the mp3 file extension
    # DONE: Loop thru the list and call get youtube title length, create a dictionary with link, title pairs
    # DONE: call download_mp3_from_youtube, and pass in the title of the video

def download_youtube_subtitles_url(subtitle_url):  # skip unittests since called by next function
...
def get_youtube_subtitles(url):  # unittests 2
    """
...
def get_youtube_all(url):  # unittests 1
    """
    Retrieves all available information from a YouTube video URL, 
    including title, length, chapters, description, and transcript. Uses yt_dlp package.
...
def is_valid_youtube_url(url):  # unittests 3
    """
    Checks if the provided URL is a valid YouTube URL by attempting to fetch video info. Uses yt_dlp package.
...
def create_youtube_md(url, title_or_path=None):  # unittests 3
    """
    Creates a markdown file with YouTube video metadata, chapters, description, and transcript.
    
    :return: path to the created markdown file.
    """
    from general.fileops import create_full_path, set_metadata_field
    from general.fileops import do_ffop, write_header_and_content_ffop, add_timestamp_links_ffop
    ...
    yt_header = "## metadata\n"  # below fields are inserted above so they app
    date_today = datetime.now().strftime("%m-%d-%Y")  # Assign today's date in format MM-DD-YYYY
    yt_header = set_metadata_field(yt_header, 'last updated', date_today + ' Created')  # Updates last updated
    yt_header = set_metadata_field(yt_header, 'link', url)
    yt_header = set_metadata_field(yt_header, 'youtube title', yt_info_dict['title'])
    yt_header = set_metadata_field(yt_header, 'youtube transcript source', yt_info_dict['transcript source'])
    yt_header = set_metadata_field(yt_header, 'length', yt_info_dict['length'])
    yt_header += "\n## content\n\n"   # the header section has the '## content' not the content section

    # TODO reconsider the need to create the new file before calling write_header_and_content_ffop and using do_ffop
    with open(yt_md_file_path, 'w') as file:
        file.write('')
    yt_md_file_path = do_ffop(write_header_and_content_ffop, yt_md_file_path, yt_header, yt_content, overwrite='yes')
    yt_md_file_path = do_ffop(add_timestamp_links_ffop, yt_md_file_path, overwrite='yes')
    return yt_md_file_path

def create_youtube_md_from_file_link(file_path):  # DS, cat 3a, unittests 3
    """
    Creates a YouTube markdown file from a given file path by extracting the YouTube link from the file's metadata.
    
    :param file_path: string of the path to the file containing the YouTube link in its metadata.
    :return: string of the path to the created YouTube markdown file.
    
    :category: 3a non ffop file function
    :area: YOUTUBE FUNCTIONS
    :usage: yt_md_file_path = create_youtube_md_from_file_link('/path/to/file_with_yt_link.md')
    """
    from general.fileops import do_ffop, sub_suffix_in_str, read_header_and_content_from_file, read_metadata_field_from_file, add_timestamp_links_ffop
    suffix_new = '_yt'

    header, _ = read_header_and_content_from_file(file_path)
    if header is None:
        raise ValueError("VALUE ERROR - header is None")

    metadata_result = read_metadata_field_from_file(file_path, "link")  # returns a tuple (line num, field val)
    if metadata_result is None or metadata_result[1] is None:
        raise ValueError(f"VALUE ERROR - 'link' metadata field is missing or None in the file: {file_path}")
    _, link = metadata_result
    yt_file_path = sub_suffix_in_str(file_path, suffix_sub=suffix_new)
...
def extract_feature_from_youtube_md(yt_md_file_path, feature):  # DS, cat 3a, unittests 3
    """
    ...
    :category: 3a non ffop file function
    :area: YOUTUBE FUNCTIONS
    :usage: print(extract_feature_from_youtube_md('/path/to/youtube_md_file.md', 'transcript'))
    """
    try:
        ...
        extracted_feature = f"{feature} (youtube)\n"  # Removed the pound signs from the feature heading
        ...
        if not feature_section_found or extracted_feature == f"{feature} (youtube)\n":
...
def test_deepgram_client():  # skip unittests
    """
    ...
    try:
        ...
        if test_deepgram_client:
            print("Successfully accessed the DeepGram API key.")
        else:
            print("Failed to access the DeepGram API key.")
    except Exception as e:
        raise ValueError(f"VALUE ERROR in test of Deepgram client: {e}")
    
def get_media_length(file_path_or_url):  # DS, cat 1, unittests 3
    """
    ...
    :usage: print(get_media_length(cur_file_path))
    """
    from general.fileops import tune_timestamp
...
def add_link_to_json(json_file_path, link):  # DS, cat 3a, unittests 3
    """ 
    ...
    :category: cat 3a non ffop file function
    :area: DEEPGRAM AND JSON FUNCTIONS
    :usage: print(add_link_to_json('/path/to/json_file.json', 'https://example.com'))
    """
    # If you want the file existence check, call from do_file_op
    try:
...
def get_link_from_json(json_file_path):  # DS, cat 3a, unittests 4
    """ 
    ...
    :category: cat 3a non ffop file function
    :area: DEEPGRAM AND JSON FUNCTIONS
    :usage: print(get_link_from_json('/path/to/json_file.json'))
    """
    # Extracts link from metadata in json
    try:
        with open(json_file_path, 'r') as file:
            data = json.load(file)
        ...
        link = None
    return link
    # DONE fill in code to extract link from metadata in json, reference code from @add_link_to_json

def transcribe_deepgram(audio_file_path, model):  # DS, cat 3a, unittests 4
    """ 
    Transcribes the given audio file using the specified Deepgram model.

    :param audio_file_path: path to the audio file to be transcribed.
    :param model: the Deepgram model to use for transcription, either 'nova-2' or 'whisper-medium'.
    :return: a dictionary containing the transcription results.

    :category: cat 3a non ffop file function
    :area: DEEPGRAM AND JSON FUNCTIONS
    :usage: print(transcribe_deepgram(cur_audio_file_path, cur_model))
    """
    # Calls the Deepgram API to transcribe
    from general.fileops import get_current_time, convert_to_epoch_seconds, get_elapsed_seconds, convert_seconds_to_timestamp, convert_timestamp_to_seconds
    ...
    if model == 'nova-2':
        ...
        raise ValueError("Invalid or absent DeepGram model ('nova-2' or 'whisper-medium' allowed).")
    ...
    try:
        ...
        print(f"Start Deepgram transcription at {start_time} for audio length of {audio_length}")
        # STEP 3: Call the transcribe_file method with the text payload and options
        # Use a timeout to prevent the write operation from timing out
        response = deepgram.listen.prerecorded.v("1").transcribe_file(payload, options, timeout=5*60*60)
        ...
        #print(response_json)  # was in example but skip because we don't want the entire json printed to the terminal
...
def get_summary_start_seconds(data, index):  # DS, cat 1, unittests 3
    """ 
    ...
    :usage: start_seconds = get_summary_start_seconds(transcription_data, word_index)
    """
...
def format_feature_segment(feature, segment, data):  # DS, cat 1, unittests 4
    """
    ...
    """
    from general.fileops import convert_seconds_to_timestamp
    ...
    # TODO add segment_midline for all features except summaries/summary, this line will be printed after the timestamp line and before the text line
    # TODO for feature='sentiments', segment_midline = '[sentiment field] - sentiment_score = [sentiment_score to 2 decimal places]'
    # TODO for feature='topics', segment_midline = '[topic field] - confidence_score = [confidence_score to 2 decimal places]'
    # TODO for feature='intents', segment_midline = '[intent field] - confidence_score = [confidence_score to 2 decimal places]'

def extract_feature_from_deepgram_json(json_file_path, feature):  # DS, cat 3a, unittests 7 and 5 (real file unittests)
    """
    ...
    :category: cat 3a non ffop file function
    :area: DEEPGRAM AND JSON FUNCTIONS
    :usage: print(extract_feature_from_deepgram_json('/path/to/json_file.json', 'summaries'))
    """
    try:
        with open(json_file_path, 'r') as file:
            data = json.load(file)
...
def validate_transcript_json(json_file_path):  # DS, cat 1, unittests 8
    """
    ...
    :category: cat 3a non ffop file function
    :area: DEEPGRAM AND JSON FUNCTIONS
    :usage: print(check_json_structure('/path/to/json_file.json'))
    """
...
def set_various_transcript_headings_ffop(file_path, feature, source, suffix_new="_set-transcript-heading", overwrite='yes'):  # DS, cat 2b, unittests 2
    """
    ...
    :param suffix_new: string of the suffix to append to the file when setting the heading. defaults to "_set-transcript-heading".
    :param overwrite: string indicating if the existing heading should be overwritten. defaults to 'yes'.
    :return: None or the result of the file operation function.

    :category: 2b nested ffop 
    :area: DEEPGRAM AND JSON FUNCTIONS
    :usage: set_various_transcript_headings_ffop('/path/to/file_dgwhspm.md', 'summaries', 'deepgram')
    """
    from general.fileops import do_ffop, set_heading_ffop, add_suffix_in_str, remove_all_suffixes_in_str, find_file_in_folders
    ...
    if source == "deepgram":
        ...
        extracted_feature = extract_feature_from_deepgram_json(dg_json_file_path, feature)
    elif source == "youtube":
        ...
        extracted_feature = extract_feature_from_youtube_md(yt_md_file_path, feature)
    else:
        raise ValueError("source invalid")
    if extracted_feature is None:
        return None
    else:
        new_heading_and_text = "### " + extracted_feature

    return do_ffop(set_heading_ffop, file_path, new_heading_and_text, "### " + feature, overwrite=overwrite)


### NUMERAL CONVERT FUNCTIONS
def extract_context(line, match, context_radius):  # DS, cat 1, unittests 5
    """ 
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print(extract_context(line, re.search("sample", line), 3))
    """
...
def print_num_exception(match_str, line_number, num_header_lines, printed_exceptions, exception_type, line):  # DS, cat 1, unittests 3
    """ 
    ...
    :param num_header_lines: the number of header lines in the file to adjust the actual line number.
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print_num_exception("42", 10, 2, printed_exceptions, "ordinal", "The answer is 42.")
    """
    match = re.search(re.escape(match_str), line)
    if match:
        exception_msg = f"Excluding conversion for {exception_type} at line {line_number+1+num_header_lines}: ...{extract_context(line, match, 5)}..."
...
def get_previous_word(substring, start_index):  # DS, cat 1, unittests 5
    """ 
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print(get_previous_word("This is a sample sentence.", 10))
    """
...
def previous_word_exception(word, common_english_vocab, additional_exception_words):  # DS, cat 1, unittests 4
    """ 
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print(previous_word_exception("Example", common_english_vocab, additional_exception_words))
    """
...
def convert_num_line_lowercase(line, num, num_str, line_number, num_header_lines, printed_exceptions, common_english_vocab):  # DS, cat 1, unittests
    """ 
    ...
    :param num_header_lines: The number of header lines in the text before the content.
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print(convert_num_line_lowercase(cur_line, cur_num, cur_num_str, cur_line_number, cur_num_header_lines, cur_printed_exceptions, cur_common_english_vocab))
    """
    num_subs_total = 0
    current_index = 0
        ...
        if line[current_index].isdigit():
            ...
            if number_str == num_str:
                ...
                if not previous_word_exception(previous_word, common_english_vocab, additional_exception_words):
                    ...
                    print_num_exception(line[current_index:end_index], line_number, num_header_lines, printed_exceptions, "proper name", line)
            current_index = end_index
        else:
            current_index += 1
...
def convert_num_line_capitalization(line, num, num_str):  # DS, cat 1, unittests 4
    """ 
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print(convert_num_line_capitalization(cur_line, cur_num, cur_num_str))
    """
    num_subs_total = 0
    for punctuation in ['.', '?', '!']:
...
def skip_speaker_line_with_single_timestamp(line):  # DS, cat 1, unittests 6
    """ 
    Determine if a line contains a single timestamp and is therefore a speaker line to skip.
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: if skip_speaker_line_with_single_timestamp(cur_line): continue
    """
...
def convert_num_lines(lines, num, num_str, num_header_lines, verbose, printed_exceptions):  # DS, cat 1, unittests 4 - could add more
    """ 
    ...
    :param num_header_lines: the number of header lines in the document to adjust line numbering for output.
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print(convert_num_lines(cur_lines, cur_num, cur_num_str, cur_num_header_lines, cur_print_output, cur_printed_exceptions))
    """
    num_subs_total = 0
    for i, line in enumerate(lines):
        if skip_speaker_line_with_single_timestamp(line):
        ...
        if num_subs > 0 and verbose:
            print(f"  convert in line {i+1+num_header_lines} the number: {num}")
        line, num_subs = convert_num_line_lowercase(line, num, num_str, i, num_header_lines, printed_exceptions, common_english_vocab)
        num_subs_total += num_subs
        lines[i] = line  # Update the line in lines
    return lines, num_subs_total

def convert_numbers_in_content(content, num_limit, additional_numbers, num_header_lines, print_output):  # DS, cat 1, unittests 2 but 2nd has 24 conversions from tests/test_manual_files/transcribe/test numbers convert.md
    """ 
    ...
    :param num_header_lines: integer representing the number of header lines in the content.
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: converted_content, total_subs = convert_numbers_in_content(content, 9, [10, 11], 2, True)
    """
    ...
    for num in list(range(num_limit)) + additional_numbers:  # Loop over all numbers from 0 to 9 and additional numbers
        ...
        lines, num_subs = convert_num_lines(lines, num, num_str, num_header_lines, print_output, printed_exceptions)
        num_subs_total += num_subs
        #print(f"Convert number {num} with conversions: {num_subs}")
    return '\n'.join(lines), num_subs_total

def convert_ordinals_in_content(content, punct_capitalization):  # DS, cat 1, unittest 4
    """ 
    ...
    :category: 1
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: print(convert_ordinals_in_content(content, ['.', '?', '!']))
    """
...
def convert_nums_to_words_ffop(file_path, suffix_new="_convertnums", verbose=False):  # DS, cat 2b,  unittests 2
    """
    ...
    :param suffix_new: string of the suffix to be appended to the original filename for the new file. Defaults to "_convertnums".
    ...
    :area: NUMERAL CONVERT FUNCTIONS
    :usage: do_ffop(convert_nums_to_words_ffop, md_file_path, overwrite='yes')
    """
    from general.fileops import verbose_print, write_header_and_content_ffop, read_header_and_content_from_file
    ...
    # TODO reconsider this error and return
    if header == None:
        print(f"ERROR because header=None in convert_nums_to_words_ffop")
        return header, content
    num_header_lines = len(header.splitlines())
    
    content = convert_ordinals_in_content(content, ['.', '?', '!'])
    converted_content, num_subs_total = convert_numbers_in_content(content, num_limit, additional_numbers, num_header_lines, verbose)
    ...
    return(write_header_and_content_ffop(file_path, header, converted_content, suffix_new))
    

### SPEAKER NAMES FUNCTIONS
def read_speaker_names_from_json(json_file_path):  # DS, cat 3a, unittests 2
    """ 
    ...
    :category: 3a non ffop file function
    :area: SPEAKER NAMES FUNCTIONS
    :usage: speakers = read_speaker_names_from_json('path/to/file.json')
    """ 
    if not os.path.exists(json_file_path):
        raise ValueError(f"The file path does not exist for {json_file_path}.")
    with open(json_file_path, 'r') as file:
...
def write_speaker_names_to_json(json_file_path, speaker_names, verbose=False):  # DS, cat 3a, unittests 3
    """ 
    Writes speaker names to a JSON file's metadata. Overwrites file with no option to create new file so not an ffop.
    ...
    :category: 3a non ffop file function
    :area: SPEAKER NAMES FUNCTIONS
    :usage: write_speaker_names_to_json('path/to/file.json', ['John', 'Bill'])
    """ 
...
def find_unassigned_speakers(md_file_path, verbose=False):  # DS, cat 3a, unittests 3
    """ 
    ...
    :category: 3a non ffop file function
    :area: SPEAKER NAMES FUNCTIONS
    :usage: unassigned = find_unassigned_speakers('path/to/markdown.md')
    """
    from general.fileops import verbose_print, get_timestamp
...
# TODO think this is done double check - change to propagate the speaker assignment backwards as well as forwards in the Markdown file.
def propagate_speaker_names_throughout_md(md_file_path, input_speaker_names=None):  # DS, cat 3a, unittests 2
    """
    ...
    :category: 3a non ffop file function
    :area: SPEAKER NAMES FUNCTIONS
    :usage: updated_speaker_names = propagate_speaker_names_throughout_md('path/to/markdown.md', [(1, 'Alice'), (2, 'Bob')])
    """
    from general.fileops import get_timestamp
...
def iterate_input_speaker_names(md_file_path, input_speaker_names=None):  # DS, cat 3a, unittests 2
    """
    ...
    :category: 3a non ffop file function
    :area: SPEAKER NAMES FUNCTIONS
    :usage: updated_speaker_names = iterate_input_speaker_names('path/to/markdown.md', [(1, 'Alice'), (2, 'Bob')])
    """
    if not os.path.exists(md_file_path):
        raise ValueError(f"The file path does not exist for {md_file_path}.")
...
def assign_speaker_names(md_file_path):  # DS, cat 3a, unittests 2 - could add more to test other cases such as 2 rounds of entries, variation in what's entered and more
    """
    ...
    :category: 3a non ffop file function
    :area: SPEAKER NAMES FUNCTIONS
    :usage: assign_speaker_names('path/to/markdown.md')
    """
...
def create_transcript_md_from_json(json_file_path, combine_segs=True):  # DS, cat 4, unittests 1
    """
    ...
    :category: 4 wrapper
    :area: WRAPPER FUNCTIONS TO PROCESS DEEPGRAM TRANSCRIPTION
    :usage: md_file_path = create_transcript_md_from_json('path/to/transcription.json')
    """
    from general.fileops import get_suffix, convert_seconds_to_timestamp, create_initial_header, set_metadata_field
    from general.fileops import do_ffop, add_timestamp_links_ffop, write_header_and_content_ffop

    md_file_path = json_file_path[:-5] + ".md"
    link = get_link_from_json(json_file_path)
    suffix = get_suffix(json_file_path, '_')
    lines = []
    ...
    header = set_metadata_field(header, 'link', link)
    header = set_metadata_field(header, 'transcript source', 'deepgram '+model_name)
    
    # TODO reconsider the need to create the new file before calling write_header_and_content_ffop and using do_ffop
    with open(md_file_path, 'w') as file:
        file.write('')
    do_ffop(write_header_and_content_ffop, md_file_path, header, content, overwrite='yes')
    do_ffop(add_timestamp_links_ffop, md_file_path, overwrite='yes')
    # do_ffop(add_dg_summaries_to_md, json_file_path, md_file_path, overwrite='yes')
    do_ffop(convert_nums_to_words_ffop, md_file_path, overwrite='yes')
    return md_file_path

def process_deepgram_transcription(title, link, model, audio_inbox_path="data/audio_inbox"):  # DS, cat 4, unittests 1 DISABLED
    """
    ...
    :category: 4 wrapper
    :area: WRAPPER FUNCTIONS TO PROCESS DEEPGRAM TRANSCRIPTION
    :usage: print(process_deepgram_transcription('Video Title', 'https://youtu.be/link', 'nova-2'))
    """
...
def process_deepgram_transcription_from_audio_file(link, audio_file_path, model):  # DS, cat 4, unittests 1 DISABLED
    """ 
    ...
    :category: 4 wrapper
    :area: WRAPPER FUNCTIONS TO PROCESS DEEPGRAM TRANSCRIPTION
    :usage: print(process_deepgram_transcription_from_audio_file('https://youtu.be/link', '/path/to/audio/file', 'nova-2'))
    """
    # Transcribe the downloaded audio file
    json_file_path = transcribe_deepgram(audio_file_path, model)
    if json_file_path is None:
...
def process_multiple_videos(videos_to_process, model='nova-2'):  # DS, cat 4, unittests 1 MOCK
    """
    Processes multiple videos by transcribing them and creating YouTube markdown files.

    :param videos_to_process: list of tuples containing the title and link of each video to be processed.
    :param model: string of the deepgram model to be used for transcription. Defaults to 'nova-2' model.
    :return: None

    :category: 4 wrapper
    :area: WRAPPER FUNCTIONS TO PROCESS DEEPGRAM TRANSCRIPTION
    :usage: process_multiple_videos([('Video Title 1', 'https://youtu.be/link1'), ('Video Title 2', 'https://youtu.be/link2')], model='nova-2')
    """
    for title, link in videos_to_process:
        try:
            cur_md = process_deepgram_transcription_from_audio_file(link, title, model)
            create_youtube_md_from_file_link(cur_md)
        except ValueError as e:
            print(f"Error processing video {title}: {e}")


# REFERENCE ONLY TO SHOW MAIN FUNCTION CALLS AND NESTING
def Xprocess_deepgram_transcription(title, link, audio_inbox_path="data/audio_inbox"):
    audio_file_path = download_mp3_from_youtube(link, audio_inbox_path + "/" + title)
    json_file_path = transcribe_deepgram(audio_file_path)
    add_link_to_json(json_file_path, link)
    md_file_path = create_transcript_md_from_json(json_file_path)
    def Xcreate_transcript_md_from_json(json_file_path, combine_segs=True):
        md_file_path = ""; header = ""; content = ""
        write_header_and_content_to_file(md_file_path, header, content)
        add_timestamp_links(md_file_path)
        add_dg_summaries_to_md(md_file_path)
        convert_nums_to_words(md_file_path, new_suffix=None, print_output=True)
    assign_speaker_names(md_file_path)


### PLAN TO DELETE
def print_progress(timeout_duration, stop_event, interval=60, long_interval=300):
    start_time = time.time()
    while not stop_event.is_set() and time.time() - start_time < timeout_duration:
        time_elapsed = time.time() - start_time
        if time_elapsed % long_interval < interval:
            print(f"{int(time_elapsed // 60)} minutes elapsed")
        else:
            print(".", end="", flush=True)
        time.sleep(interval)
    if stop_event.is_set():
        print(f"Stopped at {get_current_time_str()}")
    else:
        print(f"Timeout reached at {get_current_time_str()}")
